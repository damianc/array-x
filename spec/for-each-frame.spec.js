...
describe('forEachFrame()', () => {
  describe('should iterate over blocks of N size', () => {
    it('with smaller blocks at the end by default', () => {
      const blocks = [];
      [1,2,3,4,5].x.forEachFrame(3, block => {
...
describe('forEachFrame() with block', () => {
  ...
  it('of size 0 should perform no iteration', () => {
    const blocks = [];
    [1,2,3,4].x.forEachFrame(0, block => {
      blocks.push(block);
    });
  ...
  it('of size -1 should perform no iteration', () => {
    const blocks = [];
    [1,2,3,4].x.forEachFrame(-1, block => {
      blocks.push(block);
    });
    expect(blocks).toEqual([]);
  });
...
describe('forEachFrame() should give access', () => {
  describe('to index', () => {
  ...
  it('to original array', () => {
    const arr = [1,2,3,4];
    let ok = true;
    arr.x.forEachFrame(2, (chunk, chunkNumber, array) => {
      ok = ok && (array === arr);
    });
